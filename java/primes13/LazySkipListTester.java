...
public class LazySkipListTester {
    ...
    private static class TestThread implements Runnable {
        ...
        public TestThread(int id, long[] add, long[] find, long[] delete) {
            this.id = id;
            this.add = add;
            this.find = find;
            this.delete = delete;
    ...
    public static void main(String[] args) {
        ...
        while (numThreads < maxThreads) {
            ...
            long[] add = new long[numThreads], find = new long[numThreads], delete = new long[numThreads];
            for (int i = 0; i < numThreads; i++) {
                workers[i] = new Thread(new TestThread(i, add, find, delete));
                workers[i].start();
            }
            for (int i = 0; i < numThreads; i++) {
                try {
            ...
            for (int i = 0; i < numThreads; i++) {
                ...
                System.out.print("Average delete time: " + (delete[i] * 10 / runTimes) + "ns, ");
                System.out.println("Total run time: " + (add[i] + find[i] + delete[i]) + "ns");
            }
            System.out.println("\n");
        }
    }
